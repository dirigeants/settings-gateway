...
import type { Client, KeyedObject, ReadonlyKeyedObject } from '../types';
...
export class SettingsFolder extends Map<string, unknown> {
	...
	public async reset(
		paths: string | ReadonlyKeyedObject | readonly string[] = [...this.keys()],
		options: Readonly<SettingsFolderResetOptions> = {}
	...
	public async update(
		pathOrEntries: PathOrEntries,
		valueOrOptions?: ValueOrOptions,
	...
	private async _resolveEntry(context: SerializerUpdateContext): Promise<unknown> {
		...
		if (context.entry.array) {
			return (await Promise.all((values as readonly unknown[]).map((value) => serializer.resolve(value, context)))).filter(
				(value) => value !== null
			);
		}
	...
	private _resetSettingsFolder(changes: SettingsUpdateResult[], schemaFolder: SchemaFolder, language: Language, onlyConfigurable: boolean): void {
		...
		if (processed === 0 && skipped === 0 && nonConfigurable !== 0) throw new Error(language.get('SETTING_GATEWAY_UNCONFIGURABLE_FOLDER'));
	}
	...
	private async _processUpdate(entries: [string, unknown][], options: InternalRawFolderUpdateOptions): Promise<SettingsUpdateResults> {
		...
		const guild = client.guilds.resolve(
			typeof options.guild === 'undefined' ? ((this.base as Settings).target as GuildResolvable) : options.guild
		);
		const language = guild === null ? client.languages.default : guild.language;
		const extra = options.extraContext;
		...
		for (const [path, value] of entries) {
			...
			if (entry.type === 'Folder') {
				const keys = onlyConfigurable
					? [...(entry as SchemaFolder).values()]
							.filter((val) => val.type !== 'Folder' && (val as SchemaEntry).configurable)
							.map((val) => val.key)
			...
			promises.push(
				this._updateSettingsEntry(path, value, { entry: entry as SchemaEntry, language, guild, extraContext: extra }, internalOptions)
			);
		}
	...
	private async _updateSettingsEntry(
		key: string,
		rawValue: unknown,
		context: SerializerUpdateContext,
		options: InternalSettingsFolderUpdateOptions
		...
		const next =
			options.arrayIndex === null
				? this._updateSettingsEntryNotIndexed(previous as unknown[], await this._resolveValues(rawValue, context, false), context, options)
				: this._updateSettingsEntryAtIndex(
						previous as unknown[],
						await this._resolveValues(rawValue, context, options.arrayAction === ArrayActions.Remove),
						options.arrayIndex,
						options.arrayAction
				  );
		...
		if (options.arrayAction === ArrayActions.Auto) {
			...
			for (const value of values) {
				if (clone.includes(value))
					throw new Error(
						context.language.get('SETTING_GATEWAY_DUPLICATE_VALUE', {
							entry: context.entry,
			...
			for (const value of values) {
				...
				if (index === -1)
					throw new Error(
						context.language.get('SETTING_GATEWAY_MISSING_VALUE', {
							entry: context.entry,
							value: serializer.stringify(value, context.guild)
						})
					);
	...
	private _updateSettingsEntryAtIndex(
		...
		values: readonly unknown[],
		arrayIndex: number,
		arrayAction: ArrayActions | null
	): unknown[] {
		if (arrayIndex < 0 || arrayIndex > previous.length) {
			throw new RangeError(
				`The index ${arrayIndex} is bigger than the current array. It must be a value in the range of 0..${previous.length}.`
			);
		}
