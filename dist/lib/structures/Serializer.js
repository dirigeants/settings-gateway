    ...
    static minOrMax(value, { minimum, maximum, inclusive, key }, language) {
        if (minimum && maximum) {
            if ((value >= minimum && value <= maximum && inclusive) || (value > minimum && value < maximum && !inclusive))
                return true;
            if (minimum === maximum)
                throw new RangeError(language.get(inclusive ? 'resolverMinmaxExactlyInclusive' : 'resolverMinmaxExactlyExclusive', { name: key, min: minimum }));
            throw new RangeError(language.get(inclusive ? 'resolverMinmaxBothInclusive' : 'resolverMinmaxBothExclusive', { name: key, min: minimum, max: maximum }));
        }
        else if (minimum) {
